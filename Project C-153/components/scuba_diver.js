...
AFRAME.registerComponent("diver-rotation-reader", {
  ...
  init: function () {
    window.addEventListener("keydown", (e) => {
      ...
      if (e.key === "ArrowRight") {
        if (scubaDiverRotation.x < 10) {
          scubaDiverRotation.x += 0.5;
      ...
      if (e.key === "ArrowUp") {
        if (scubaDiverRotation.z < 20) {
          scubaDiverRotation.z += 0.5;
          this.el.setAttribute("rotation",  scubaDiverRotation);
        }
        if (scubaDiverPosition.y < 2) {
          scubaDiverPosition.y += 0.01;
          this.el.setAttribute("position",  scubaDiverPosition);
        }
      }
      if (e.key === "ArrowDown") {
        if (scubaDiverRotation.z > -10) {
          scubaDiverRotation.z -= 0.5;
          this.el.setAttribute("rotation",  scubaDiverRotation);
        }
        if (scubaDiverPosition.y > -2) {
